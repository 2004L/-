...
- 语音终端支持说出或输入手机号后四位，并在复述确认后精确匹配订单；身份识别不使用向量近似匹配。
- 正常流程按“订单匹配 → 身份核验 → 锁房 → 隔离浏览器登记模拟 → 等待现场人员发卡”推进，并把状态和审计事件写回数据库。
- 重复订单、已入住、已取消和未找到等情况会阻止 AI 擅自放行；未找到时可创建一笔现场演示订单。
- 支付争议、设备故障、退房保洁、公网页面验证码或证书异常，可通过统一事件契约通知对应部门。

当前演示只覆盖“持有效中国居民身份证的成年人正常入住”的交互原型。未成年人、境外客人、特殊证件、证件损坏/不符、复杂账务和真实公安/支付/门锁操作都必须由正式业务系统和人工流程接管。演示结束状态为 `READY_FOR_ONSITE_HANDOFF`，不会伪造“房卡已发出”或“客人已入住”。
...
`app/api/demo/[action]/route.ts` 提供按会话隔离的假数据、精确匹配、状态转移、浏览器任务和审计接口。首次打开会自动写入假订单；管理后台可以恢复初始数据。

| 手机后四位 | 场景 | 预期结果 |
| --- | --- | --- |
| `4821` | 美团待入住 | 完整跑通，到“等待现场人员发卡” |
| `6395` | 抖音团购待入住 | 完整跑通 |
| `9053` | 现场待入住 | 完整跑通 |
...
所有姓名、手机号、订单号、身份 token 和登记回执均为演示数据。`browser-start` 与 `browser-complete` 只驱动隔离的模拟任务，不连接真实公安系统。
