...
- AI Native 语音终端支持自由表达，意图层会把“查预订、现场入住、早餐、停车、押金、退房和房卡进度”等自然语言对齐到受控业务动作；线上订单使用手机号后四位精确匹配，现场办理先识别并确认完整手机号，不使用向量近似匹配身份。
- 正常流程按“订单匹配 → 身份证感应与自动读取 → 身份核验 → 锁房 → 隔离浏览器登记模拟 → PMS 入住确认 → 发卡机自动写卡、回读和吐卡 → 取卡确认”推进，并把状态、设备回执和审计事件写回数据库。
- 重复订单、已入住、已取消和未找到等情况会阻止 AI 擅自放行；现场办理必须先登记完整手机号，再走草稿、报价和支付确认。
...
## 第二阶段：现场办理先报价、再支付、后建单

现场到店不是直接写入正式订单，而是按以下顺序推进：

```text
完整手机号确认 → walk_in_draft 草稿 → 房型/晚数/间数 → 后端报价
       → 客人确认金额 → 微信/支付宝模拟支付 → 支付回执确认
       → 创建正式现场订单 → 订单匹配 → 身份证/公安/发卡流程
```

`walk_in_drafts` 和 `walk_in_payments` 是独立的临时数据。手机号只保存后四位、掩码和 SHA-256 令牌；报价由后端房型表计算，前端不能传入金额；支付接口使用幂等键，重复点击不会重复扣款或建单。支付状态不是 `PAID` 时，服务端拒绝创建正式 `demo_orders`，也不会进入身份证读取、公安登记或房卡流程。

演示房型和价格仅用于验收：标准大床房 ¥260/晚、押金 ¥200；高级大床房 ¥380/晚、押金 ¥300；豪华双床房 ¥420/晚、押金 ¥300。微信和支付宝目前返回模拟支付码，后续接入真实支付时只替换支付适配器，草稿、报价、幂等和状态门禁保持不变。

新增演示接口：

| 能力 | 方法与路径 | 关键约束 |
| --- | --- | --- |
| 创建现场草稿 | `POST /api/demo/walk-in-draft` | 必须是完整手机号；不建正式订单 |
| 生成报价 | `POST /api/demo/walk-in-quote` | 房型、晚数、间数由后端校验和计算 |
| 生成支付页 | `POST /api/demo/walk-in-payment` | 只能对 `QUOTED` 草稿创建待支付单 |
| 确认支付 | `POST /api/demo/walk-in-payment-complete` | 仅 `PENDING` → `PAID`；成功后才建正式订单 |

所有阶段均写入脱敏审计事件：`WALK_IN_DRAFT_CREATED`、`WALK_IN_QUOTED`、`PAYMENT_STARTED`、`PAYMENT_CONFIRMED`、`WALK_IN_ORDER_CREATED`。支付回执不确定或并发重复请求时，系统保持原状态并要求人工核对，不猜测成功。

## 设计边界与安全原则

### 管理员认证与角色权限（第一阶段）

管理后台必须先登录，服务端通过 8 小时有效的 HttpOnly、Secure、SameSite=Strict 会话识别管理员。当前演示预置老板、店长、前台、客房四种角色；权限在接口层校验，前端隐藏按钮不构成安全边界。管理员登录、失败、权限拒绝和退出会写入脱敏的 `admin_audit_events`。管理员工具只允许通过 `lib/admin-tools.ts` 的契约调用，模型不能直接写数据库。

详细的角色权限、演示账号和生产替换要求见 [`docs/admin-auth.md`](docs/admin-auth.md)。
...
每次对话会保留最近 24 条脱敏消息，并把工具结果摘要带入下一轮，让模型能理解“刚才那笔订单”“继续办理”等说法。现场办理意图在第一阶段只完成完整手机号的识别与确认，不提前创建订单或收款；后续选房、支付和登记由业务状态机接管。工具名和参数先通过 schema 校验，再交给业务 API 或仿真器执行；模型不会直接访问数据库。

模型回复通过 AI SDK `streamText` 以 SSE 增量返回，前端会边收到边显示；工具调用会先缓冲完整参数并再次通过 schema 校验，只有最终的 `tool_call` 才会进入现有业务接口。若模型未配置、流式接口不可用或请求超时，接口仍返回本地规则结果，入住状态机不受影响。
