...
export function DigitalHuman({ input, onInput, audioInputs, selectedAudioDeviceId, onAudioDeviceChange, onToggleComputerUse, onToggleContinuousConversation }: {
  ...
  return <aside className="digital-human-card fixed left-2 top-20 z-30 w-[min(30rem,34vw)] overflow-visible text-left md:left-6 md:top-24 lg:left-8 max-md:bottom-3 max-md:top-auto max-md:w-[calc(100vw-1rem)] max-md:max-w-[25rem]">
    <div className="flex items-center justify-between px-2 py-2">
      ...
      <div className="flex items-center gap-1 rounded-full bg-white/80 p-1 shadow-sm backdrop-blur-md">
        <button type="button" onClick={onToggleContinuousConversation} className={`rounded-full px-2.5 py-1.5 text-[11px] font-medium ${input.continuousConversation ? "bg-[#eaf4ff] text-[#1769aa]" : "text-[#6e6e73] hover:bg-[#f5f5f7]"}`} aria-pressed={input.continuousConversation}>{input.continuousConversation ? "连续通话" : "按键说话"}</button>
    ...
    <div ref={containerRef} className="relative h-[26rem] overflow-visible md:h-[34rem] lg:h-[39rem]">
      ...
      {input.listening && <div className="absolute left-2 top-3 flex items-center gap-2 rounded-full bg-[#1d1d1f]/75 px-3 py-1.5 text-[11px] text-white shadow-md"><Mic size={12} /><span>{input.voiceFinalizing ? "正在整理" : "正在听"}</span><span className="h-1.5 w-12 overflow-hidden rounded-full bg-white/25"><span className="block h-full rounded-full bg-[#64d2ff]" style={{ width: `${Math.max(8, Math.round(input.audioLevel * 100))}%` }} /></span></div>}
    </div>
    <div className="grid grid-cols-3 gap-2 px-2 py-2">
      <button type="button" onClick={() => emit({ type: input.listening ? "voice_stop" : "voice_start" })} disabled={isBusy || input.voiceFinalizing} className={`flex items-center justify-center gap-1.5 rounded-xl px-2 py-2.5 text-xs font-medium ${input.listening ? "bg-[#ff3b30] text-white" : "bg-[#eaf4ff] text-[#1769aa]"} disabled:opacity-40`}><Mic size={14} />{input.voiceFinalizing ? "识别中" : input.listening ? (input.continuousConversation ? "结束本句" : "结束") : (input.continuousConversation ? "开始通话" : "说话")}</button>
    ...
    <div className="flex items-center justify-between px-3 pb-2 pt-2 text-[10px] text-[#9a9aa1]"><span>{input.computerUseStatus === "waiting_confirmation" ? "等待用户点击确认" : input.continuousConversation ? "连续通话已开启 · 说完自动发送" : input.computerUseEnabled ? "AI 可操作当前页面 · 高风险仍需确认" : "数字人输入与传统确认共用同一办理内核"}</span><Volume2 size={12} /></div>
  </aside>;
}
