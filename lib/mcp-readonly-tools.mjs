...
const MCP_RETRY_DELAYS_MS = [100, 250];
...
export function createHotelApiClient(config, fetchImpl = fetch) {
  ...
  async function read(path) {
    ...
    for (let attempt = 0; attempt < MCP_MAX_ATTEMPTS; attempt += 1) {
      try {
        ...
        if (!retryableStatus(response.status) || attempt === MCP_MAX_ATTEMPTS - 1) throw error;
      } catch (error) {
        if (attempt === MCP_MAX_ATTEMPTS - 1 || (error?.code === 'upstream_error' && !retryableStatus(error.status))) throw error;
      }
      await wait(MCP_RETRY_DELAYS_MS[attempt] ?? MCP_RETRY_DELAYS_MS.at(-1));
    }
    throw new Error('mcp_upstream_retry_exhausted');
  }
  return { read };
}
