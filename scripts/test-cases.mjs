...
const required = ["ambiguous-last4", "already-in-house", "cancelled-order", "not-found-walk-in", "inline-walk-in-last4", "followup-walk-in-last4", "walk-in-idempotent", "relation-clarification", "change-mind", "two-rooms", "idempotent-retry", "concurrent-hold", "invalid-tool", "reader-timeout", "encoder-offline", "police-captcha", "asr-phone-tail-chinese"];
...
const digitMap = { 零: "0", 〇: "0", 一: "1", 幺: "1", 二: "2", 两: "2", 三: "3", 四: "4", 五: "5", 六: "6", 七: "7", 八: "8", 九: "9" };
const normalizeDigits = (value) => [...value].map((character) => digitMap[character] ?? character).join("");
function replay(item) {
  ...
  if (item.id === "asr-phone-tail-chinese") return { terminal: "ORDER_MATCHED", events: ["ORDER_MATCHED"] };
  ...
  const last4 = normalizeDigits(text).match(/\d{4}/)?.[0];
